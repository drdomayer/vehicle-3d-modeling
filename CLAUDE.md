...
- 2026-09-15 (local, 31): **v018 — refinement pass. ТЕКУЩА БАЗА.** Четири точки, две от които се
  ...
  **Преден калник:** излишъкът над v016 намален от +161 на +85 mm при specX 350 след шест
  ...
  който ги разделя, и той насища. Не е върнато на 678, защото короната на арката е 673.5 и 678
  оставяше 4.5 mm тяло над отвора. Цена: процепът при A-стълба 114 → 134 mm; от specX 1000 назад
  линията не мърда изобщо.
...
- **Текуща база: `02_DESIGN/exterior/STATEV_001_v018.blend`. Откат: v017.**
  Авторитетни източници: геометрия — v017.blend, регенерируем от `statev_master_volumes.py`;
  измервания — `04_ENGINEERING/reports/v018_report.txt`; решения — `docs/09-decision-log.md`;
  заключени визуални решения — `docs/14-locked-visual-decisions.md`. Всеки доклад с версия в
  името е исторически и НЕ е текущ.
- Next: собственикът решава две неща, преди да продължим — дали +85 mm на предния калник се
  приема, и дали задният undercut да се омекоти (препоръка B). Нито една зона не е замразена — CHECKPOINT 01 не е минат и собственикът не е дал одобрение на силуета.
