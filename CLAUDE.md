...
- 2026-09-21 (local, 10): **край до край: сглобявам обратно отпечатаните файлове и меря колата.**
  ...
  **99.4% в 8 mm**; **колата не е цяла** — покритие 35.5%.
  ...
  Намерен и дефект по заключен размер: **ядрото излизаше 3.5 mm извън 925**, защото 5–15% от лицата
  на всеки панел гледат навътре, а `solidify` строи стената обратно на нормалата. `face_outward()`
  го поправя — **но Blender падна по време на проверяващия пробег, значи поправката НЕ е потвърдена.**
  Изходите в `03_PRINT/` са от преди нея и са частично презаписани; **първото при следващо отваряне
  е пълен пробег и проверка на 925.00.**

- **Текуща база: `02_DESIGN/exterior/STATEV_001_v035.blend`. Откат: v034.**
