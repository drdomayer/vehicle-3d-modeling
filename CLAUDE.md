...
- **Текуща база: `02_DESIGN/exterior/STATEV_001_v030.blend`. Откат: v029.**
...
- Next: **решение по прага** (`docs/14` раздел D — 47° при G1 лента 8–40; или лентата е твърде
  тясна, или преходът се връща на ~18 mm), после **двата CHECK на арките**: отворите са
  26.2 mm (предна) и 24.8 mm (задна) по-широки от построената повърхност, тоест има отвор без
  повърхност за рязане. И двете числа са НАШИ, не донорски, значи се решават без скан —
  задната повърхност е на 924.2 при таван 925, значи отворът се стеснява до повърхността,
  не обратното. След това **етап 02 — реални повърхности по зони**, patch-базирани по
  `docs/16`, при спазена карта на непрекъснатостта 5×G0 / 10×G1 / 2×G2. Това е единственото, което стои между регистъра от 38
