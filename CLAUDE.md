...
- **Текуща база: `02_DESIGN/exterior/STATEV_001_v017.blend`. Откат: v016.**
  Авторитетни източници: геометрия — v017.blend, регенерируем от `statev_master_volumes.py`;
  измервания — `04_ENGINEERING/reports/v017_report.txt`; решения — `docs/09-decision-log.md`;
  заключени визуални решения — `docs/14-locked-visual-decisions.md`. Всеки доклад с версия в
  името е исторически и НЕ е текущ.
- Next: pass по петте останали точки върху v017 (front fender top, хлътването в канала при
  specX 715–820, ръбът на задния deck, фасетата на носа, преценка за задния undercut). Нито една
  зона не е замразена — CHECKPOINT 01 не е минат и собственикът не е дал одобрение на силуета.
