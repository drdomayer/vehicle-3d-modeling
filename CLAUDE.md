...
- 2026-09-26 (local, 5): **припуск 30 mm през донорските shut-линии за shape-only панелите.**
  `trim_at()` носи повърхността на съседа 30 mm отвъд линията, без рампа. Проверено от файловете:
  P03 до specX 500 (линия 440), P09 418–1667, P15 от 1600; двойките идентични. Shape-only съседите
  нарочно се застъпват с 30 mm — след скана се реже, не се премоделира. Цена: люспи 59 → 73.
  444 файла, 13 non-manifold, точност и ширина непроменени, 23 доклада current.

- 2026-09-26 (local, 6): **надвесите измерени, class-A нагоре на всяка секция.** `overhang_report.py`
  дава на Q32 брой и литри вместо неизвестна — след три бъга в самия него, хванати от числата му
  (единици, лицето на плота броено за надвес, и генератор, затворил върху последния триъгълник).
  Мери и **обем на опората** (площ × разстояние до плота), защото ъгълът не различава 1 mm от
  100 mm. Находка: `lay_flat` никога не е питал коя страна нагоре — половината секции печатаха
  външната кожа върху опори. Сега **class-A лицето е нагоре на 444 от 444** (`CLASS_A_UP` в
  графика); цената е 232 L / 278 L опори, съзнателна размяна финиш срещу опори. 24 доклада current.

- 2026-09-26 (local, 8): **v037 — рамената отпред и отзад мерени срещу ref-09 и поправени.**
  Нова мярка `endview_overlay.py` (горен контур на предния/задния изглед, външната четвърт,
  нормализирано): референцията е стена → ъгъл → плато; моделът беше рампа под ~50°. Отпред
  **0.180 → 0.033** (гребен на y 780 вместо 600, 40 под короната; силуетът отстрани го носи
  `HOOD_SPINE`; връх на стената + плато като котви), отзад **0.189 → 0.078** (`FLANK_TOP` 800,
  `FLANK_TOP_HOLD` 0.45, `HAUNCH_PLATEAU` на нивото на deck-а, навън, нищо свалено). Двата
  междинни опита отзад са записани с числата, с които паднаха. `check_goal.py` пази двете нови
  метрики. Осемте стари: непроменени в допуск. Ръбове 5 / 2 / 0.
  Верига: **крехка след удебеляване** (`P03 s14b`, 12 триъгълника, ширина 1853.6 → **1850.0**;
  50 файла-трохи по-малко) и **нечистени директории** (578 файла срещу 425 части; `print_qc`
  одитираше 153 сираци — вече чисти преди експорт и чете само `placement.json`). Медиана 0.00,
